...
export const IdCardGenerator = async (fullName, role, batch, standard, section, rollNo, dob, address, imageName, schoolFolderName) => {
  ...
  const createFrontIdCard = async (fullName, role, batch, standard, section, rollNo, dob, address) => {
    const existingPdfBytes = await fetch("http://localhost:5000/storage/idCardFrontDesign.pdf").then((res) => res.arrayBuffer());
    ...
    //get font
    const MontserratMediumFile = await fetch("http://localhost:5000/storage/Montserrat-Medium.otf").then((res) => res.arrayBuffer());
    const MontserratBoldFile = await fetch("http://localhost:5000/storage/Montserrat-Bold.ttf").then((res) => res.arrayBuffer());
    ...
    await setImageToPdfWithBorder(`http://localhost:5000/storage/${schoolFolderName}/${imageName}`, 170, 345, 160);
    ...
    await setImageToPDFWithoutBorder(pdfDoc, firstPage, "http://localhost:5000/storage/schoolLogo.png", 63, 570, 125);
    // await setImageToPDFWithoutBorder(pdfDoc, firstPage, "http://localhost:5000/storage/barcode.JPG", 45, 60, 170);
