...
const AcademicInfoForm = () => {
  ...
  const removeRow = (tableId: string) => {
  ...
  const overrideSN = () => {
    const updatedRows = tableRows.map((row, index) => ({ ...row, sn: index + 1 }));
    setTableRows(updatedRows);
  };

  const handleInputChange = (event: ChangeEvent<HTMLInputElement>, rowIndex: number, field: string) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      if (localStorage.getItem("academicInformation")) {
        setTableRows(JSON.parse(localStorage.getItem("academicInformation")!) as TableRow[]);
      }
    }
  }, []);
      ...
      <main>
        <section className='detail-row-section'>
          <h2 className='title roboto_500'>
            <span className='required-field'> Marks Obtained in SEE or Equivalent </span>
          </h2>

          <div className='table-btn-container'>
            <button className='table-btn' onClick={() => addRow("table1")}>
              <i className='bx bx-plus'></i>
            </button>
            <button className='table-btn' onClick={() => removeRow("table1")}>
          ...
          <section className='application-form-footer'>
            <div className='footer-btn-container'>
              <a
                onClick={() => {
                  router.push("/admissionPortal/gradeXI/step2");
                }}
