  ...
  useEffect(() => {
    for (let i = 0; i < document.getElementsByClassName("test-upload").length; i++) {
      ...
      document.getElementsByClassName("test-upload")[i].addEventListener("change", function (e) {
        ///@ts-ignore
        const testPreview = document.getElementsByClassName("test-preview")[i] as HTMLCollectionOf<HTMLElement>;
        if (testPreview && testPreview.length > 0) {
          ///@ts-ignore
          testPreview[0].innerHTML = ` <p id='upload-success-msg' className='one-liner' style="color: #0f5288">${e.target.files[0].name}</p>`;
        }
      });
    }
  }, []);
